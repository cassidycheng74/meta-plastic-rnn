"""
analysis/plot_training_curves_30task.py

Plot per-task performance as a function of training steps
for the 30-task LeakyRNN baseline run.

Key fix in this version: checkpoint evaluation injects task identity
vectors into channels 7-10, matching what TaskDataset does during
training. Without this, tasks like delayanti show 0.0 performance
even when they are learned.

Usage:
    python analysis/plot_training_curves_30task.py

Reads from:
    runs/LeakyRNN_256units_30tasks_seed0/

Saves figures to:
    runs/LeakyRNN_256units_30tasks_seed0/figures/
"""
...
# ---------------------------------------------------------------------------
# Config — update RUN_DIR and N_RNN to match the run you want to analyse
# ---------------------------------------------------------------------------

RUN_DIR  = 'runs/LeakyRNN_256units_30tasks_seed0'
N_RNN    = 256     # must match the checkpoint
SEED     = 0       # must match the TaskDataset seed used during training
...
print(f'Perf avg (logged, may be inaccurate): {perf_avg[0]:.3f} -> {perf_avg[-1]:.3f}')
...
ax = axes[1]
ax.plot(steps, perf_avg, color='#4dac26', linewidth=2, label='avg (logged)')
ax.plot(steps, perf_min, color='#d7191c', linewidth=2,
        linestyle='--', label='min (logged)')
ax.set_xlabel('Training Steps (thousands)', fontsize=11)
ax.set_ylabel('Performance', fontsize=11)
ax.set_title('Performance (avg and min)\nNote: logged values may be inaccurate\n'
             'if eval did not inject task identity', fontsize=10)
...
plt.suptitle(f'30-Task LeakyRNN Training Summary\n'
             f'({N_RNN} units, lr=3e-4, {steps[-1]:.0f}k steps)',
             fontsize=13, y=1.02)
plt.tight_layout()
...
# ---------------------------------------------------------------------------
# Load checkpoints with correct task identity injection
# ---------------------------------------------------------------------------

print('\nLoading per-task performance from checkpoints...')
print('(injecting task identity vectors to match training)')

config    = make_config(n_rnn=N_RNN, batch_size=16)
all_tasks = {**YANG_DRISCOLL_TASKS, **NEW_TASKS}

# Recreate task identity vectors using same seed as training.
...
# Sample at most 20 checkpoints for speed.
N = max(1, len(ckpt_files) // 20)
...
for ckpt_file in sampled_ckpts:
    ...
    # If task_vecs were saved in the checkpoint, use those.
    # Otherwise fall back to seed-based reconstruction.
    if 'task_vecs' in ckpt:
        task_vecs = ckpt['task_vecs']
    else:
        task_vecs = dataset.task_vecs
    ...
    with torch.no_grad():
        for name, fn in all_tasks.items():
            ...
            for _ in range(3):
                trial = fn(config, rng)
                # Inject task identity vector — critical fix.
...
# ---------------------------------------------------------------------------
# Figure 2: Per-family performance curves
# ---------------------------------------------------------------------------
...
plt.suptitle(f'Per-Task Performance by Family\n'
             f'30-Task LeakyRNN ({N_RNN} units, {ckpt_steps[-1]:.0f}k steps)',
             fontsize=13, y=1.01)
plt.tight_layout()
...
# ---------------------------------------------------------------------------
# Figure 3: All tasks in one plot
# ---------------------------------------------------------------------------

final_perfs  = {name: task_perfs[name][-1] for name in all_tasks if task_perfs[name]}
...
for task in sorted_tasks:
    ...
    ax.plot(ckpt_steps, task_perfs[task], color=color, alpha=0.7, linewidth=1.5)
...
ax.set_title('All 30 Tasks: Performance over Training\n'
             '(sorted by final performance, colored by task family)', fontsize=12)
ax.set_ylim([-0.05, 1.05])
...
bar_tasks, bar_perfs, bar_colors, bar_labels = [], [], [], []
...
bars = ax.bar(x, bar_perfs, color=bar_colors, edgecolor='white', linewidth=0.5)
ax.set_xticks(x)
...
ax.set_title(f'Final Performance Across All 30 Tasks\n'
             f'(at step {ckpt_steps[-1]:.0f}k, {N_RNN} units)',
             fontsize=12)
ax.set_ylim([0, 1.15])
ax.axhline(1.0, color='gray', linestyle='--', linewidth=0.8, alpha=0.5)
...
for bar, perf in zip(bars, bar_perfs):
    ax.text(bar.get_x() + bar.get_width() / 2,
            bar.get_height() + 0.02,
            f'{perf:.2f}', ha='center', va='bottom', fontsize=7)
...
print('\n' + '=' * 50)
print('FINAL PERFORMANCE SUMMARY (with task identity)')
print('=' * 50)
print('%-25s %8s %8s' % ('Task', 'Final', 'Max'))
print('-' * 45)
for task in sorted_tasks:
    perfs = task_perfs[task]
    print('%-25s %8.3f %8.3f' % (
        TASK_LABELS.get(task, task), perfs[-1], max(perfs)))
